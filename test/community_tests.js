...
import { adminRole, buyer2Role, sellerB1Role, randomPersonRole } from './pages/roles'
import {
  myUrl,
  group,
  link,
...
import { checkErrors, createGroup, prepareForPressKey } from './pages/helper'
...
test('Create public group. Group names cannot be duplicated', async (t) => {
  ...
  await t
    .navigateTo(myUrl + '/dashboard/posts')
  ...
  await t
    .expect(groupsPage.buttons.editGroup.exists).ok()

  await t
    .hover(groupsPage.buttons.editGroup)
  ...
  await t
    ...
    .expect(Selector('h1').withText(group.commonName).exists).ok(
        `group title ${group.commonName} exists on page details`
    )
...
test('Rate question and answer', async (t) => {
  ...
  await t
    .hover(topicsPage.vote.pointUpQuestion)
    .click(topicsPage.vote.pointUpQuestion) // rate the question

  await t
    .hover(topicsPage.vote.pointUpAnswer) // rate the answer)
    .wait(100)
  ...
  await t
    ...
    .ok("Expects question rate increased by 1")

  await t
    .expect(topicsPage.ratings.firstAnswer.exists)
    .ok("Expects answer rate increased by 1")
    .setTestSpeed(1)
})
